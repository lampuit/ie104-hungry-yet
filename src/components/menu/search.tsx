...
export function SearchingArea({ totalAmount, filter, onFilterChange }: SearchingAreaProps) {
    ...
    return (
        <div className="flex flex-col items-center justify-between px-4 py-6 space-y-4 sm:flex-row sm:space-y-0 sm:space-x-4">
            <div className="flex flex-col items-center space-y-2">
                <h2 className="text-3xl font-semibold italic text-gray-800 sm:text-4xl">Thực đơn</h2>
            ...
            <div className="flex items-center justify-end space-x-2 sm:space-x-4 w-full sm:w-auto">
                <div className={`fixed inset-x-0 top-0 z-50 flex items-center justify-between p-4 bg-white shadow-md transition-all duration-300 ease-in-out sm:relative sm:inset-auto sm:p-0 sm:shadow-none sm:flex-1 ${isSearchVisible ? 'translate-y-0' : '-translate-y-full sm:translate-y-0'}`}>
                    <input
                        type="text"
                        value={search}
                        onChange={handleSearchChange}
                        className="w-full px-4 py-2 text-sm border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent sm:w-64"
                ...
                <div className="flex items-center space-x-2">
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="outline" size="icon" className="text-gray-600 hover:text-gray-800 focus:outline-none">
                                <Filter size={20} />
                            </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent className="w-56 p-2 bg-white rounded-lg shadow-lg">
                    ...
                    <Link href="/menu/cart" className="relative">
                        <div className="relative p-2 text-gray-600 bg-gray-100 rounded-full hover:bg-gray-200">
