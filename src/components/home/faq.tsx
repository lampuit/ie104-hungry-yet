...
export function FAQ() {
  ...
  return (
    <Accordion
      type="single"
      collapsible
      className="flex flex-col justify-center mx-auto w-[320px] sm:w-[360px] gap-4 md:w-[540px] lg:w-[480px] xl:w-[600px]"
    >
      {faqItems.map((item, index) => (
        <AccordionItem
          key={`item-${index + 1}`}
          value={`item-${index + 1}`}
          className="w-full rounded-2xl border-2 border-[#99BD76] shadow-md shadow-[#99BD7680]"
        >
