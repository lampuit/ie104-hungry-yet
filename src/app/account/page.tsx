...
export default function Account() {
  ...
  useEffect(() => {
    ...
    setShortName(
      userInfo && userInfo[0]?.name ? splitName(userInfo[0].name) : "",
    );
  }, [userInfo]);
  ...
  return isLoading ? (
    ...
    <div className="flex flex-col gap-2 px-2 sm:gap-5 sm:px-4">
      <h2 className="text-xl font-semibold sm:text-2xl">Tài khoản của tôi</h2>
      <Form {...form}>
        <form
          onSubmit={form.handleSubmit(onSubmit)}
          className="flex w-full flex-col items-center rounded-md border-2 bg-white p-4 lg:flex-row lg:items-start lg:justify-start lg:gap-16 lg:p-16"
        >
          {/* Avatar Section */}
          <div className="flex w-1/2 flex-col items-center gap-6 lg:w-1/4">
            <Avatar className="size-32">
              <AvatarImage
                src={
                  selectedFile
                    ? URL.createObjectURL(selectedFile)
                    : (userInfo?.[0]?.imageUrl ?? undefined)
                }
              />
              <AvatarFallback className="text-4xl">{shortName}</AvatarFallback>
            </Avatar>
            <Label htmlFor="avatar-upload" className="cursor-pointer">
              <div className="flex items-center gap-2 rounded-md border border-amber-500 px-4 py-2 text-xs text-amber-500 hover:bg-amber-500 hover:bg-opacity-20 md:text-sm">
                <SquarePen className="h-4 w-4 stroke-amber-500" />
                Thay đổi ảnh
              </div>
            </Label>
            <Input
          ...
          <div className="flex w-full flex-col gap-6 lg:w-3/4">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <div className="text-xs font-semibold sm:text-sm">Email:</div>
                  <FormControl>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="space-y-3">
                  <FormLabel className="text-xs font-semibold sm:text-sm">
                    Giới tính:
                  </FormLabel>
                  <FormControl>
                    <RadioGroup
                  ...
                  <FormControl>
                    <DatePicker
                      ...
                      selected={
                        ...
                        field.value?.day
                          ? new Date(
                              Number(field.value.year),
                              Number(field.value.month) - 1,
                      ...
                      onChange={(date) => {
                        if (date) {
                          ...
                          form.setValue("birthday", {
                            ...
                            month: "",
                            year: "",
                          }); // Nếu không có ngày, đặt lại giá trị
                        }
                      }}
            ...
            <Button
              type="submit"
              className="w-full bg-amber-500 hover:bg-red-500 sm:w-1/2"
            >
              Lưu thay đổi
            </Button>
          </div>
